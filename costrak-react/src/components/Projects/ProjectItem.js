...
function ProjectItem(props) {
    const projectURL = `/projects/${props.title.split(' ').join('-').toLowerCase()}?id=${props.id}`
    return (
            ...
            <div className="service-item">
                ...
                <div className="service-content">
                    <div className="service-content-inner">
                        <h5>
                            <Link to={`${process.env.PUBLIC_URL + projectURL}`} className="stretched-link">{props.title}</Link>
                        </h5>
                        <p>{props.text}</p>
                    </div>
                </div>
